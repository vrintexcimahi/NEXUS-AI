...
function setHistoryStore(value) {
  window.storageSetJson(getHistoryKey(window.currentUser), value || {});
}
...
async function sendMessage(customText = null, options = {}) {
  ...
  if (window.pywebview?.api) {
    window.pywebview.api.send_message(text).then(res => {
      updateAiMessage(res);
...
function createAiMessagePlaceholder() {
  ...
  msgDiv.innerHTML = `
    ...
    <div class="msg-content" id="current-ai-content" style="padding:0; background:none; border:none; box-shadow:none;">
      <div class="typing">
